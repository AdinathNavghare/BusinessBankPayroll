...
//for insert into EMP_MAST2 to EMPMAST
 /*
  try {
	...
	ResultSet rs=null;
	ResultSet rs1=null;
	rs1=st3.executeQuery("select * from [NAMCO_DBF].[dbo].[EMP_MAST2] order by join_date ");
	...
	rs=st1.executeQuery("select EMP_CODE from [NAMCO_DBF].[dbo].[PERSONL1]  where EMP_CODE ='"+rs1.getString("EMP_CODE")+"'");
	int count =1;
	while(rs.next())
	{
		...
		st2.execute(" BEGIN "+
			...
			"	  set @EMPCODE= (SELECT E1.EMP_CODE FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+   
			"	  set @fullname= (SELECT E1.EMP_NAME FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
			"     set @oldfullname= (SELECT E1.OLD_NAME FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
			...
			"	  SET @GENDER= (SELECT  CASE WHEN 'M' = (SELECT E1.SEX   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)  THEN 'M' ELSE 'F' END)"+ 
			"	  SET @Married=(SELECT E1.MAR_STAT   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1  WHERE E1.EMP_CODE LIKE @CONDITION) "+
			"	  SET @salute= (SELECT  CASE WHEN 'M' = (SELECT E1.SEX   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)  THEN 1 ELSE (select case when 'M'=@Married or 'W'=@Married then 2 else 3 END) END)"+
			"     SET @CAST=(select isnull((SELECT E1.CASTE from [NAMCO_DBF].[dbo].[PERSONL1] E1  WHERE E1.EMP_CODE LIKE @CONDITION),7))"+
			"     SET @CAT=(select isnull((SELECT E1.CASTE_CD from [NAMCO_DBF].[dbo].[PERSONL1] E1  WHERE E1.EMP_CODE LIKE @CONDITION),10))"+		
			"     SET @REL=(select isnull((SELECT E1.RELIGION from [NAMCO_DBF].[dbo].[PERSONL1] E1  WHERE E1.EMP_CODE LIKE @CONDITION),2))"	+	
			"     SET @DOJ=(SELECT JOIN_DATE from [NAMCO_DBF].[dbo].[emp_mast2] where emp_code= @CONDITION) "+
			...
			"     SET @EMPLOYEE_TYPE=(SELECT E1.TYPE   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1  WHERE E1.EMP_CODE LIKE @CONDITION) "+		
			"	  SET @DOB=(SELECT E1.BIRTH_DT   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) "+
			"     SET @blood=(select LKP_SRNO FROM LOOKUP WHERE LKP_CODE= 'BLOOD_GRP' and LKP_DISC like ( select isnull((select BLD_GRP from [NAMCO_DBF].[dbo].[PERSONL1] E1  WHERE E1.EMP_CODE LIKE @CONDITION),'BLOOD_GRP')))"+	  
			"     SET @PFNO=(SELECT E1.PF_NO   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+ 
			"	  SET @handi=(SELECT E1.HANDICAP   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) "+	 
			"	  SET @PANNO=(SELECT E1.PAN_NO   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) "+
			"     SET @aadhar= (SELECT  E1.adhar_no FROM [NAMCO_DBF].[dbo].[PMJJ1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
			""+
		...
		el.errorLog("EMP_MAST2 to EMPMAST trial.jsp",e.toString());
	}
...
*/
    ...
    /*
     // for remaining employees for insert into EMP_MAST2 to EMPMAST
  try {
	Connection con=ConnectionManager.getConnection();
	Statement st1=con.createStatement();
	Statement st2=con.createStatement();
	...
	rs=st1.executeQuery("select * from [NAMCO_DBF].[dbo].[EMP_MAST2]  where EMP_CODE  not in(select empcode from empmast) order by join_date ");
	int count =1;
	while(rs.next())
	{
		...
		st2.execute(" BEGIN "+
			...
			"	  set @fullname= (SELECT E1.EMP_NAME FROM [NAMCO_DBF].[dbo].[EMP_MAST2] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
			"    set @oldfullname=''"+
			"	  set @LNAME= LEFT(@fullname, CharIndex(' ', @fullname) - 1)   "+
			"	  SET @MNAME=SUBSTRING(@fullname, CharIndex(' ', @fullname), LEN(@fullname) - charindex(' ', REVERSE(@fullname)) - charindex(' ', @fullname) + 1)"+	
			"	  SET @FNAME=   reverse(left(reverse(@fullname), charindex(' ', reverse(@fullname)) -1))"+ 
			...
			"     SET @CATEGORYCD=(select LKP_SRNO FROM LOOKUP WHERE LKP_CODE='CATE' and LKP_DISC like @CAT)"+
			"     SET @RELEGENCD=(select LKP_SRNO FROM LOOKUP WHERE LKP_CODE= 'RELIG' and LKP_DISC like @REL)"+			
			"     SET @EMPLOYEE_TYPE=(SELECT E1.TYPE   FROM [NAMCO_DBF].[dbo].[EMP_MAST2] E1  WHERE E1.EMP_CODE LIKE @CONDITION) "+		
			"	  SET @DOB=(SELECT E1.BIRTH_DT   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) "+
			"     SET @blood=(select LKP_SRNO FROM LOOKUP WHERE LKP_CODE= 'BLOOD_GRP' and LKP_DISC='BLOOD_GRP')"+
			" set  @DESIG=(SELECT desg_cd from [NAMCO_DBF].[dbo].[EMP_MAST2] where emp_code= @CONDITION) "+
			" set @PRJ_SRNO=(SELECT br_cd from [NAMCO_DBF].[dbo].[EMP_MAST2] where emp_code= @CONDITION) "+	
...
*/
 ...
 /*    
//for single from EMP_MAST2 to empmast value of adhar number
      try {
		Connection con=ConnectionManager.getConnection();
		Statement st1=con.createStatement();
		Statement st2=con.createStatement();
		...
		ResultSet r1=st2.executeQuery("select CONVERT(varchar(100), Cast(ADHAR_NO as decimal(38, 0))) as adhar_no   FROM [NAMCO_DBF].[dbo].[PMJJ1]   WHERE EMP_CODE LIKE '"+rs.getString(1)+"'");
		while(r1.next())
		{
...
//for insert into EMP_MAST2 to EMPTRAN
 try {
	Connection con=ConnectionManager.getConnection();
	Statement st1=con.createStatement();
	Statement st2=con.createStatement();
	ResultSet rs=null;
	int count =1;
	rs=st1.executeQuery("select distinct empcode from empmast where empcode in (select emp_code from [NAMCO_DBF].[dbo].[EMP_MAST2] where  join_date is not null)");
	while(rs.next())
	{
     ...
     "     declare @DESIG varchar(100)"+  
      ...
      "     declare @PRJ_CD varchar(100)"+  
       "     declare @EFFDATE varchar(100)"+ 
         ...
         "        declare @EMPNO varchar(100)"+
          "             SET @CONDITION='"+rs.getString(1)+"'   "+
           "            SET @EMPNO=(SELECT EMPNO FROM EMPMAST WHERE EMPCODE LIKE @CONDITION)"+  
            "           set @DEPT=(select LKP_SRNO FROM LOOKUP WHERE LKP_CODE='DEPT' AND LKP_DISC=((SELECT ISNULL((SELECT E1.DEPT_CD   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),'DEPARTMENT'))))"+   
             "          SET @DESIG= (SELECT E1.DESG_CD   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
              "         SET @PRJ_SRNO=(SELECT E1.BR_CD   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
              "         SET @PRJ_CD=(SELECT E1.project_code   FROM PROJECT_SITES E1 WHERE E1.SITE_ID =@PRJ_SRNO)"+
               "        SET @EFFDATE= (SELECT E1.JOIN_DATE   FROM [NAMCO_DBF].[dbo].[EMP_MAST2] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
                "       SET @ACNO=(SELECT E1.BANK_AC   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) "+
		...
		e.printStackTrace();
		ErrorLog el=new ErrorLog();
		el.errorLog("EMP_MAST2 to EMPTRAN trial.jsp",e.toString());
	}
   ...
   try {
		...
		System.out.println(rs.getString(1));
			st2.execute(" BEGIN "+
				...
				"  SET @CONDITION='"+rs.getString(1)+"' "+
				"	  SET @empno=  (select empno  from empmast where  EMPCODE LIKE @CONDITION)"+
				" SET @ADDR1= (select isnull((SELECT E1.P_ADDR1   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),''))"+
				" SET @ADDR2=(SELECT isnull((SELECT E1.p_ADDR2   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),''))"+
					  
					  
		"			  SET @ADDR3=(SELECT E1.P_ADDR3   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) +' ,'"+
		"			  SET @city=(select lkp_srno from lookup where lkp_code='CITY' and lkp_DISC=(select isnull((SELECT E1.p_CITY  FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),'CITY')))"+
		"			  SET @PIN=(SELECT E1.P_PINCD   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+
		"			  SET @st=(SELECT LKP_RECR  FROM LOOKUP where lkp_CODE='CITY' and lkp_SRNO=@city)"+
					  
		"			  SET @ph=(select isnull((SELECT E1.P_PHONE   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),0))"+ 
		"			  INSERT INTO EMPAUX (EMPNO,ADDR1,ADDR2,PIN,TELNO,ADDRTYPE,STATE) "+
		"			                VALUES(@empno,@ADDR1+@ADDR2,@ADDR3+@city,@PIN,@ph,'PA',@st)"+
					  
					   
		" SET @ADDR1= (select isnull((SELECT E1.t_ADDR1   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),''))"+
		" SET @ADDR2=(SELECT isnull((SELECT E1.t_ADDR2   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),''))"+
		"			   SET @ADDR3=(SELECT E1.t_ADDR3   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION) +' ,'"+
		"			  SET @city=(select lkp_srno from lookup where lkp_code='CITY' and lkp_DISC=(select isnull((SELECT E1.T_CITY  FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),'CITY')))"+
				
		"			  SET @PIN=(SELECT E1.t_PINCD   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION)"+ 
					  
		"			  SET @ph=(select isnull((SELECT E1.t_PHONE   FROM [NAMCO_DBF].[dbo].[PERSONL1] E1 WHERE E1.EMP_CODE LIKE @CONDITION),0))"+ 
		"			  INSERT INTO EMPAUX (EMPNO,ADDR1,ADDR2,PIN,TELNO,ADDRTYPE,STATE) "+
		"			  VALUES(@empno,@ADDR1+@ADDR2,@ADDR3+@city ,@PIN,@ph,'CA',@st) "+
			...
			e.printStackTrace();
			ErrorLog el=new ErrorLog();
			el.errorLog("PERSONL1 to EMPAUX trial.jsp",e.toString());
		}
...
//For QUAL FROM QUAL_DTL1  
  ...
  /*  try {
		...
		Statement st2=con.createStatement();
		ResultSet rs=null;
		rs=st1.executeQuery("select emp_cd,srno from [NAMCO_DBF].[dbo].[QUAL_DTL1] order by emp_cd,srno");
		int count =1;
		while(rs.next())
		{
		System.out.println(rs.getString(1));
			          ...
			          "       declare @PASSYEAR varchar(100)"+
			           "            SET @CONDITION='"+rs.getString(1)+"' "+
			            "           SET @EMPNO=(SELECT EMPNO FROM EMPMAST WHERE EMPCODE LIKE '"+rs.getString(1)+"')					"+  
			             "          set @REM=  (SELECT SUBJECT FROM  [NAMCO_DBF].[dbo].[QUAL_DTL1] WHERE EMP_CD LIKE @CONDITION and srno="+rs.getString(2)+")  "+
			              "         SET @CLASS=(SELECT CLASS FROM  [NAMCO_DBF].[dbo].[QUAL_DTL1] WHERE EMP_CD LIKE @CONDITION and srno="+rs.getString(2)+") "+		
			               "        SET @DEGREE= (SELECT QUAL_CD   FROM  [NAMCO_DBF].[dbo].[QUAL_DTL1] WHERE EMP_CD LIKE @CONDITION and srno="+rs.getString(2)+")"+
			                "       SET @INST=(SELECT  UNIV_INST   FROM  [NAMCO_DBF].[dbo].[QUAL_DTL1] WHERE EMP_CD LIKE @CONDITION and srno="+rs.getString(2)+")"+
			                 "      SET @PER= (SELECT PERCENTAGE   FROM  [NAMCO_DBF].[dbo].[QUAL_DTL1] WHERE EMP_CD LIKE @CONDITION and srno="+rs.getString(2)+")"+
			                  "     SET @PASSYEAR=(SELECT YEAR_PASS  FROM  [NAMCO_DBF].[dbo].[QUAL_DTL1] WHERE EMP_CD LIKE @CONDITION and srno="+rs.getString(2)+")"+
			...
			e.printStackTrace();
			ErrorLog el=new ErrorLog();
			el.errorLog("QUAL_DTL1 to QUAL trial.jsp",e.toString());
		}
...
stmt1.execute("insert into vdamast"+  
...
try {
	...
	ResultSet rs2;
	ResultSet rs1;
	String table =" [NAMCO_DBF].[dbo].[SAL15081] ";
	String table1="PAYTRAN";
	String dt="31-Aug-2015";
	String mnth="Aug-2015";
	String empcode="";
	String empno="";
	int trncd;
	String col="";
rs1=stmt.executeQuery("select empcode,empno from empmast where empcode in (select distinct emp_code from  "+table+" ) order by empno ");	
...
while(rs2.next())
{
	...
	if(trncd!=999 && trncd!=998)
	{
		...
		{
			trncd=999;
			stmt3.execute(" begin insert into "+table1+" (TRNDT,EMPNO,TRNCD,SRNO,INP_AMT,CAL_AMT,ADJ_AMT,ARR_AMT,NET_AMT,CF_SW,USRCODE,UPDDT,STATUS)"+
					"values('"+dt+"',"+empno+","+trncd+",0,"+rs2.getFloat(1)+","+rs2.getFloat(1)+",0.0,0.0,"+rs2.getFloat(1)+",'','','"+dt+"','P') end ");
			stmt3.execute(" begin insert into "+table1+" (TRNDT,EMPNO,TRNCD,SRNO,INP_AMT,CAL_AMT,ADJ_AMT,ARR_AMT,NET_AMT,CF_SW,USRCODE,UPDDT,STATUS)"+
					"values('"+dt+"',"+empno+",199,0,"+rs2.getFloat(1)+","+rs2.getFloat(1)+",0.0,0.0,"+rs2.getFloat(1)+",'','','"+dt+"','P') end ");
			stmt3.execute(" begin insert into "+table1+" (TRNDT,EMPNO,TRNCD,SRNO,INP_AMT,CAL_AMT,ADJ_AMT,ARR_AMT,NET_AMT,CF_SW,USRCODE,UPDDT,STATUS)"+
					"values('"+dt+"',"+empno+",198,0,"+rs2.getFloat(1)+","+rs2.getFloat(1)+",0.0,0.0,"+rs2.getFloat(1)+",'','','"+dt+"','P') end ");
...
stmt3.execute(" begin insert into [NAMCO_PAYROLL].dbo.sal_details "+ 
"(EMPNO,SAL_MONTH,SAL_STATUS,SAL_PAID_DATE)"+
" values("+empno+",'"+mnth+"','PROCESSED','"+dt+"')  end ");
  ...
  try {
	 ...
	 */  
	  ...
	  int count=1;
		...
		Statement st2=con.createStatement();
		Statement st3=con.createStatement();
		String select="select * from [NAMCO_DBF].[dbo].[LEV_DTL] where FROM_DT is not null and from_dt>='1-Jan-2015' order by  EMP_CODE";
		...
		ResultSet rs= st1.executeQuery(select);
			...
			String empno="";
			String selectSecond="select empno from [NAMCO_FOR_TEST].[dbo].[EMPMAST] where empcode like '"+rs.getString("EMP_CODE")+"'";		
			System.out.println(selectSecond);
			ResultSet r=st3.executeQuery(selectSecond);			
		while(r.next())
		{
			empno=r.getString("EMPNO");
			System.out.println(empno);
		}
		String sql=" insert into  [NAMCO_FOR_TEST].[dbo].[LEAVETRAN] " +
				"([LEAVECD],[EMPNO],[TRNDATE],[TRNTYPE],[APPLNO],[LREASON],[APPLDT],[FRMDT],[TODT],[STATUS],[DAYS])"+
				" VALUES("+LEAVECD+","+empno+",'"+rs.getString("APP_DATE")+"','D',(select isnull((select max(applno)+1 from [NAMCO_FOR_TEST].[dbo].[LEAVETRAN]),1) )"+
				" ,'"+rs.getString("REASON")+" ','"+rs.getString("SAN_DATE")+"','"+rs.getString("FROM_DT")+"','"+rs.getString("TO_DT")+"','"+(rs.getString("SANCTION").equalsIgnoreCase("Y")?"SANCTION":"1")+"',"+rs.getString("LDAYS")+")";
...
catch(Exception e)
{
  ...
  */
  ...
  try {
		 int count=1;
			...
			String table =" [NAMCO_DBF].[dbo].[SAL15041] ";
			...
			st2.execute(" insert into  [NAMCO_FOR_TEST].[dbo].[LEAVEBAL] " +
			"([BALDT],[EMPNO],[LEAVECD],[BAL],[TOTCR],[TOTDR])"+
			" VALUES('"+dt+"',"+empno+",1,"+pl+","+op_pl+","+(op_pl-pl)+")");	
			
			st2.execute(" insert into  [NAMCO_FOR_TEST].[dbo].[LEAVEBAL] " +
					"([BALDT],[EMPNO],[LEAVECD],[BAL],[TOTCR],[TOTDR])"+
					" VALUES('"+dt+"',"+empno+",3,"+cl+","+op_cl+","+(op_cl-cl)+")");	
			
			st2.execute(" insert into  [NAMCO_FOR_TEST].[dbo].[LEAVEBAL] " +
					"([BALDT],[EMPNO],[LEAVECD],[BAL],[TOTCR],[TOTDR])"+
					" VALUES('"+dt+"',"+empno+",4,"+ml+","+op_ml+","+(op_ml-ml)+")");	
			}	
				}
  ...
  */
   ...
   try {
		...
		ResultSet rs= st1.executeQuery("select * from [NAMCO_DBF].[dbo].[LIC_MST1] order by EDATE ");
 			while(rs.next())
 				{
				...
				String date1 = rs.getString("EDATE")==null?"1900-01-01":rs.getString("EDATE");
				
				
				String sql="insert into DEDMAST  ([EMPNO],[TRNCD],[SRNO],[AMOUNT],[SUBSYS_CD],[AC_NO],[BODSANCNO],[SANC_DATE],"+
...
================================== 
	...
	ResultSet rs= st1.executeQuery("select * from [NAMCO_DBF].[dbo].[LOAN_MST] order by EDATE ");
 				while(rs.next())
 				{
 					 ...
 					 if(rs.getInt("trncd")==1){trncd="245";}
 					else if(rs.getInt("trncd")==2){trncd="257";}
 					else if(rs.getInt("trncd")==3){trncd="253";} 
 					else if(rs.getInt("trncd")==4){trncd="255";}
					else if(rs.getInt("trncd")==5){trncd="246";}
					else if(rs.getInt("trncd")==6){trncd="244";}					
					else if(rs.getInt("trncd")==7){trncd="248";}
					else if(rs.getInt("trncd")==8){trncd="249";}
 				ResultSet rs1=st3.executeQuery("select empcode,empno from empmast where empcode LIKE '"+rs.getString("EMP_CD")+"'");	
				while(rs1.next())
 					{
					empno=rs1.getString("empno");
 ...
 */
   ...
   // Insert into reliving info
	...
	try
	{
		...
		st2.execute("insert into namco_FOR_TEST.dbo.RELIEVEINFO(EMPNO,LEFT_DATE,RESGN_DATE,RESGN_ACCTD_DATE,REASON,NTC_PERIOD,TERMINATE,DEATH) "+ 
		" values("+empno+",'"+dt+"','"+dt+"','"+dt+"','','NA','Yes','')");
		Statement stupdate = conn.createStatement();
		stupdate.executeUpdate("update namco_FOR_TEST.dbo.EMPMAST SET DOL = '"+dt+"',STATUS = 'N', UMODDATE ='"+dt+"' where EMPNO ="+empno+"");
