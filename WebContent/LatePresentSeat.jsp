...
int Lateempno = Integer.parseInt(request.getParameter("EMPNO"));
EmployeeBean employeeBean=new EmployeeBean();

employeeBean = employeeHandler.getEmployeeInformation(Integer.toString(Lateempno));	
System.out.println("employeeBean.getSi"+employeeBean.getSite_id());
String date1=request.getParameter("date");
	tran = eoffhdlr.getLateEmpList(request.getParameter("EMPNO"));
...
System.out.println(".......................................");
Emp_al=EAH.getEmpAttend(date1, Lateempno);
...
HolidayMasterHandler holidayMasterHandler= new HolidayMasterHandler();
//prjCode = Integer.parseInt(request.getParameter("prj")==null?"0":request.getParameter("prj"));
System.out.println("prjcode"+prjCode);
weekdays=holidayMasterHandler.getweekoff(date1,employeeBean.getSite_id());
holidays = holidayMasterHandler.getHoldmast(date1,employeeBean.getSite_id());
...
$(document).ready(function () {
    ...
    $('#tranSave').click(function () 
    	...
    	var check=confirm("Are you sure to save the changes?");
    		});
...
function inputLimiter(e,allow) {
		  ...
		  if (allow == 'Numbers'){AllowableCharacters='Ll';}
		  var k;
...
function attd()
{
	...
	if(r==true){
	window.location.href="LateMark.jsp";
	}
	else{
		return false;
	}
...
function checkFlag() {
	...
	if (f == 1) {
		alert("Late marks saved Successfully");

	}
	else if (f == 2) {
		alert("Problem in saving Late marks");
	}
}
			...
			<div id="table-content">
				...
				<center>
									...
									<form action="EmpAttendServlet?action=insert"method="post" onsubmit="validate()">
													...
													<tr>
													<th colspan="3">Employee Late Mark Details</th>
													</tr>
														<tr>
																<td> Date :
																...
																<td>	<input type="submit" class="myButton" style="margin-right: 20px;" id="tranSave" value="Save" disabled="disabled"/>
																 <input type="button" class="myButton" style="margin-right: 20px;" value="Edit"   id="edit" height="150" width="150" />
																 <input type="Button" class="myButton" value="Back"  onclick="attd()"> </td>
												...
												<br>
												<h3>DAYS-
												<input type="button" class="myButton"  onclick="fn('div2','div1')" value="1-15" title="Click on the button to view Result of date 1 to 15 !"/>
												<input type="button" class="myButton" onclick="fn('div1','div2')" value="16-above" title="Click on the button to view Result of date 16 to above  !"/>
												</h3>
										
												<div style="height: 330px; width: 1180px;">
											...
											<div id="div<%=start%>" align="center" style=" height: 360px;width: 1150px;">
												...
												<% }%>
													<table id="customers" style="width: 90%"; >
														...
														<tr style="height: 35px;">
																...
																<th style="width: 6%;" > <input type="text"  disabled="disabled"  onclick="this.click()" readonly="readonly" style=" width: 100%;   background-color: #1F5FA7; color: white;border:none; text-align: center;"  name="SR.No." value="SR.No" ></th>
																<th style="width: 8%;" > <input type="text"   disabled="disabled"  onclick="this.click()" readonly="readonly" style="width: 100%;  background-color: #1F5FA7; color: white;border:none;  text-align: center;"  name="Emp Code" value="Emp Code" ></th>
																<th style="width: 22%;" > <input type="text"   disabled="disabled"  onclick="this.click()" readonly="readonly" style=" width: 100%; background-color: #1F5FA7; color: white;border:none;  text-align: center;"  name="Emp name" value="Employee Name" ></th>
																<%
																for(int i=s;i<=e;i++)
																{
																%>
															 	<th style="width: 4%;"> <input type="text" size="2" maxlength="2"  disabled="disabled"  onclick="this.click()" readonly="readonly" style=" width: 100%; background-color: #1F5FA7; color: white;border:none;  text-align: center;"  name="SR.No." value= <%=i<=9?"&nbsp;"+i:i%> ></th> 
																<%
																}
															...
															for(int j=0;j<Emp_al.size();j++)
															{
    															...
    															//put the month u want
    															   ...
    															   // auto generated sundays code ends here...........!!!!!!!!!!!!
																	...
																	if(c+1==d1||c+1==d2||c+1==d3 )
																	{
																		...
																		<input type="text" size="4" name="_<%=ab.getEMPNO()%>" id="<%=z++%>" maxlength="2" value="WO"
																...
																System.out.println("check"+check);
																			...
																			
																			{
																				%>	
																				<td style="width: 4%;" align="center">
																				<input type="text" size="4" name="_<%=ab.getEMPNO()%>" id="<%=z++%>" maxlength="2" value="HD" readonly="readonly"	
																			 	disabled="disabled" onfocus="this.select()" title="Date= <%=c+1%>" style="width: 80%;color:black; text-align: center;text-transform: uppercase;" onkeypress="return inputLimiter(event,'Numbers')">
																				</td>
																				<%
																			}
																		...
																		<input type="text" size="4"  name="_<%=ab.getEMPNO()%>" id="<%=z++%>" maxlength="1" value="<%=ab.getVal()==null?"P":ab.getVal()%>" disabled="disabled"
																	 ...
																	 readonly="readonly"	 disabled="disabled" onfocus="this.select()" title="Date= <%=c+1%>" style="width: 80%;color:black; text-align: center;text-transform: uppercase;" onkeypress="return inputLimiter(event,'Numbers')">
																		...
																		<%
																			...
																			if(check)
																			{
																				%>	
																				<td style="width: 4%;" align="center">
																				<input type="text" size="4" name="_<%=ab.getEMPNO()%>" id="<%=z++%>" maxlength="2" value="HD"
																			 readonly="readonly"		disabled="disabled" onfocus="this.select()" title="Date= <%=c+1%>" style="width: 80%;color:black;text-align: center;text-transform: uppercase;" onkeypress="return inputLimiter(event,'Numbers')">
																				</td>
																				<%
																			}
																		...
																		<td style="width: 4%;" align="center">
																		<input type="text" size="4"  name="_<%=ab.getEMPNO()%>" id="<%=z++%>" maxlength="1" value="<%=ab.getVal()==null?"P":ab.getVal()%>"
																	 disabled="disabled" onfocus="this.select()" title="Date=   <%=c+1%>" style="width: 80%;color:black;text-align: center;text-transform: uppercase;" onkeypress="return inputLimiter(event,'Numbers')">
																		</td>
										...
										<input type="button" name="Button" value="Fill ALL Late" title="Click on the button to feel the present in all !" disabled="disabled" onclick="return method()"/> 					
